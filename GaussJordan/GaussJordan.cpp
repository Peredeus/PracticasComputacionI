...
void GaussJordan(matriz & miMatriz) {
    ...
    for (int k = 0; k < variables; k++) {
        may = abs(miMatriz[k][k]);
        ind = k;

        for (int l = k + 1; l < variables; l++) {
            if (may < abs(miMatriz[l][k])) {
                may = abs(miMatriz[l][k]);
                ind = l;
            }
        ...
        if (miMatriz[k][k] == 0) {
            ...
            for (int i = 0; i < variables; i++) {
                if (i != k) {
                    pivote = -miMatriz[i][k];
                    for (int j = k; j < variables + 1; j++) {

                        miMatriz[i][j] = miMatriz[i][j] + pivote * miMatriz[k][j] / miMatriz[k][k];
                    }
                } else {
                    pivote = miMatriz[k][k];
                    for (int j = k; j < variables + 1; j++) {
                        miMatriz[i][j] = miMatriz[i][j] / pivote;
                    }
                }
            }
        }
